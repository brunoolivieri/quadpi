...
def goto(vehicle, target_location):
    ...
    distance_to_target_location = get_distance_meters(target_location,vehicle.location.global_relative_frame)
    vehicle.simple_goto(target_location)

    while vehicle.mode.name=="GUIDED":
        current_distance = get_distance_meters(target_location,vehicle.location.global_relative_frame)
        if current_distance < 0.5:  # 0.5 meters error
            print("Reached target waypoint at %s" % str(vehicle.location.global_relative_frame).split(':')[1].replace(',', ', ') )
            time.sleep(1) # time to copter cover the error
            break
        else:
            print('Distance for the next way point: %s meters.' % current_distance)
            time.sleep(1)

    return None

def travel_through_waypoints(vehicle, way_points):
