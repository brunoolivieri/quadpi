...
config.read('/etc/gradys_config.ini')
...
if __name__ == "__main__":
    ...
    copter = Copter(sysid=int(config['mesurer']['sysid']))
    copter.connect(connection_string=str(config['mesurer']['connection_string']))

    print("Asking GUIDED mode...")
    copter.change_mode("GUIDED")

    print("Waiting be ready to arm...")
    ...
    print("Asking takeoff to 20 meters...")
    copter.user_takeoff(conf_experiment_high)

    print("Waiting to reach at least 15m...")
    copter.wait_altitude((conf_experiment_high -5), (conf_experiment_high + 5), True, timeout=60)
    ...
    copter.mav.mav.set_position_target_local_ned_send(
        ...
        1,  # targetm component id
        mavutil.mavlink.MAV_FRAME_LOCAL_NED,  #- Positions are relative to the vehicle’s EKF Origin in NED frame
        #mavutil.mavlink.MAV_FRAME_LOCAL_OFFSET_NED, #- Positions are relative to the vehicle’s current position
        #mavutil.mavlink.MAV_FRAME_BODY_NED, #- Positions are relative to the EKF Origin in NED frame
        #mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED, #- Positions are relative to the vehicle’s current position and heading
        ...
        0,  # afy
        0,  # afz
        conf_experiment_heading,  # yaw
        0,  # yawrate
    )

    print("Sleeping for experiment loop...")
    time.sleep(conf_experiment_sleeps)
    ...
    for num in range(1, conf_experiment_tests +1):
            ...
            copter.mav.mav.set_position_target_local_ned_send(
                ...
                3576, # mask
                conf_experiment_distance_between_tests,  # x
                0,  # y
                0,  # z
                conf_experiment_speed,  # vx
                0,  # vy
                0,  # vz
                0,  # afx
                0,  # afy
    ...
    print('Solicitando RTL ...')
    copter.do_RTL()
    print('Esperando RTL ...')
    copter.wait_rtl_complete()
